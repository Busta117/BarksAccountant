...
│       │   │   ├── Sale.kt          # data class Sale(id, clientName, orderDate, deliveryDate?, products, totalPrice, isPaid, isDelivered)
...
│       │       ├── stats/           # StatsStore: business performance dashboard
│       │       │   ├── StatsState.kt, StatsMessage.kt, StatsEffect.kt, StatsStore.kt
│       │       └── settings/
│       │           ├── products/
...
│                   ├── stats/
│                   │   └── StatsScreen.kt         # Stats dashboard: filters + 5 sections
│                   └── settings/
...
│       ├── MainTabView.swift         # TabView: Ventas, Compras, Stats, Settings (with onLogout)
...
│       ├── Stats/
│       │   ├── StatsView.swift            # Stats dashboard: filters + 5 sections
│       │   └── StatsStoreWrapper.swift    # @Observable wrapper for StatsStore
│       └── Settings/
...
- Sale IDs: auto-incremental format `YYYY-NNNN` (e.g., `2026-0001`). Generated by querying existing sales for the current year and incrementing. Other entity IDs use `kotlin.uuid.Uuid.random().toString()` (`@OptIn(ExperimentalUuidApi::class)`)
...
- 4 tabs: Ventas (sales_list), Compras (purchases_list), Stats (stats), Settings (settings)
...
- **Important**: Only one `.navigationDestination(for: T.self)` per type per NavigationStack. Settings uses typed enums (`SettingsDestination`, `ProductDestination`, `ClientDestination`) to avoid conflicts.
...
- Sales/ is PBXGroup D10004, Purchases/ is D10005, Stats/ is D10007, Settings/ is D10006 — all under BarksAccountantApp group (D10002)
...
- **Detail**: all sale info (no responsable field), mark as paid (alert confirm), mark as delivered (alert confirm, sets deliveryDate to today), edit button pushes form
- **Form (create/edit)**: client picker (sheet on iOS, dialog on Android), date pickers (order + optional delivery), products list with +/- quantity (- at 1 = delete/trash icon), add product picker, total, save button, **delete button with confirmation** (only in edit mode)
- **Sale IDs**: auto-incremental format `YYYY-NNNN` (e.g., `2026-0001`). Generated at save time by querying Firestore for existing sales of the current year and incrementing the max number.
...
### Stats (iOS + Android): COMPLETE
- **Dashboard**: filtered by year (required) and optionally by month. All data fetched once from Firestore, stats computed client-side via getters on StatsState.
- **Filter**: year dropdown + month dropdown ("Todos" = full year). Year change resets month to null.
- **Section 1 "Resumen"**: total sales, total purchases, net profit (green/red), margin %
- **Section 2 "Indicadores"**: sales count, average ticket, unpaid total, undelivered count
- **Section 3 "Desglose mensual"**: per-month totals (only shown when no month filter, only months with data)
- **Section 4 "Productos más vendidos"**: product name, units sold, revenue (sorted by quantity desc)
- **Section 5 "Principales clientes"**: client name, order count, total amount (sorted by amount desc)
- Each section shows "Sin datos para mostrar" if empty
- Shared: StatsStore with computed getters — no extra network calls on filter change
...
## Known Issues / Notes
- Android `MainActivity`: `serviceLocator.appId` must be set synchronously in composition (not in `LaunchedEffect`) to avoid Firestore path errors (`apps//sales`)
- `kotlin.uuid.Uuid` requires `@OptIn(ExperimentalUuidApi::class)` — used in InMemory repos and non-sale Firestore repos
